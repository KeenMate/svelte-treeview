...
- nodeTitleOverflow: NodeTitleOverflow (default 'wrap') - 'wrap' | 'ellipsis' | 'info'. What the built-in label (.stv__node-label) does when wider than the row. wrap = multi-line (row grows); ellipsis = single-line clip; info = ellipsis + a trailing circled-ⓘ that shows ONLY on rows whose label is actually clipped (per-node scrollWidth>clientWidth measure, re-run on content change + shared-ResizeObserver box change), click reveals the full label in a @floating-ui popover (position:fixed, dismiss on 2nd click/outside/scroll/Esc; click never selects) + sets native title on clipped rows. wrap/ellipsis are pure CSS gated by data-node-title-overflow on .stv__container (flex min-width:0 fixes). Only the default label path — a nodeTemplate snippet owns its own layout. Ported from web-multiselect's fullscreen clipped-label affordance. The ⓘ glyph is the Lucide `info` icon via CSS mask (inherits color), overridable via --stv-icon-info. CSS hooks .stv__node-info / .stv__node-info-reveal (+ --stv-icon-info, --stv-node-info-*). Guarded by e2e/node-title-overflow.spec.ts + fixture /test/node-title-overflow; demo /examples/responsive RS03.
