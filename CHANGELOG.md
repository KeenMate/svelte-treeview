...
### Added
- **`ltree-highlight-anchor` visual marker for the Shift+click anchor**: The node at `lastHighlightedPath` (the "point of origin" for the next Shift+click range) now gets the new `.ltree-highlight-anchor` class — a 3 px primary-colored left-border by default. Tunable via `--ltree-highlight-anchor-width` and `--ltree-highlight-anchor-color`. New `isHighlightAnchor: boolean` flag on `LTreeNode`. Previously the anchor was tracked internally but had no visual representation, so users couldn't tell where a Shift+click range would start from.
