...
## [5.0.0-rc13] - 2026-06-30 [PUBLISHED]

Clipboard machinery overhaul: an immutable clipboard snapshot, a per-node paste transform that carries live node references (no display-name assumption), a copy-time clean hook, leaf-aware paste positioning, skip-aware (no silent all-or-nothing) pasting, and the `beforeCopy` / `beforeCut` / `beforePaste` interceptors brought to a consistent reference-passing shape. Plus a multi-drag focus-follow fix and demo parity (cross-tree clipboard, `Copy N` naming, `Delete` with a guard).

### Changed
- **Paste data-flow rebuilt around an immutable clipboard + a per-node transform.** The module-level clipboard is now immutable to consumers: `pasteNodes` deep-copies the entries into a per-paste working copy, so the persistent snapshot (a copy can be pasted repeatedly) is never mutated — eliminating the foot-gun where `beforePasteCallback` rewriting `entry.data` for "Copy N" naming persisted onto the clipboard and needed a strip-`" Copy N"` regex to avoid compounding into `Copy 1 Copy 1`. Per-node derivation (fresh ids/values/names, and skipping) moves to a new **`pasteNodeTransformationCallback(data, ctx) => T | null`** — return `null` to skip a node (skipping a root skips its subtree). Its context passes **live node references** — `sourceNode` (the original, same-tree), `targetParent` (destination parent, null at root) and `siblings` (the destination's existing children, batch-aware so it includes nodes added earlier in the same paste) — instead of a precomputed `takenNames` list, so the consumer decides what "collision" means by reading any field off the sibling nodes, with **no `displayValueMember` requirement**. The old per-call `transformData(data, index, op)` arg is replaced by this richer `(data, ctx)` shape (still accepted as an optional `pasteNodes` override; otherwise the `<Tree>` prop is used). `PasteResult` gains `skipped`. New exported `uniqueName(base, taken, suffix?)` helper, and new exported types `PasteNodeTransformContext<T>`, `CopyNodeTransformContext`, `BeforePasteContext<T>`, `BeforeCopyContext<T>`.
- **`beforePasteCallback` is batch-policy-only and takes a context object.** `(targetPath, operation, entries)` → `(ctx: BeforePasteContext<T>)` with `ctx = { targetPath, targetNode, operation, entries }`. It now receives the **resolved target node** (`targetNode`, null at root), so a redirect reads `ctx.targetNode?.parentPath` instead of reaching for the tree ref or slicing the path string. `entries` is `readonly` immutable snapshots by design (sources may be cross-tree, or removed on a cut); per-node data rewriting belongs in the transform, not here.
- **`beforeCopyCallback` / `beforeCutCallback` take a context object.** `(paths: string[])` → `(ctx: BeforeCopyContext<T>)` with `ctx = { operation, paths, nodes }` — the resolved live `nodes` are provided alongside the paths, for symmetry with `beforePaste` / `beforeDrop`. (`beforeDropCallback` already passed node references — `dropNode` / `draggedNode` — and is unchanged.)
...
- **`/examples/drag-drop`**: cross-tree clipboard (`Ctrl/Cmd + C / X / V` between the two trees — copy duplicates, cut moves and removes the source) and collision-aware `Copy N` naming on paste.
- **E2E**: new `/test/clipboard-transform` fixture + `e2e/clipboard-transform.spec.ts` (copy redaction, transform-`null` skip + `skipped` count, parent+child partial paste, leaf-aware paste, `Delete` guard); plus a multi-drag focus-follow regression in `e2e/drag-drop.spec.ts`.
