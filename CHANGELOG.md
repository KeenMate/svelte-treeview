...
- **Windows File Explorer demo at `/examples/custom-layout`** (`WindowsExplorer.svelte`): a near-complete dual-pane Explorer clone built on `<Tree>` to showcase real-app composition. The left nav pane is a `<Tree>` fed a folders-only slice of the data (`leafIconClass=""` so folders without subfolders show no marker, like Explorer); the right pane is a custom sortable details list (Name / Date modified / Type / Size) of the selected folder's children (folders + files). The two panes stay in sync through the tree's public API — `expandNodes()` to reveal ancestors and `focusNode()` to highlight — driven from `onNodeClick` (tree → contents) and the contents pane's double-click / breadcrumb / Back-Forward-Up history (contents → tree). The dataset models a real `C:\Windows` install (~30 folders, multi-level subfolders + files). Also: per-column sorting (folders-first); **recursive search** across the current folder's subtree with each hit's location shown and **wildcard globs** (`*.dll`, `img?.jpg`); single / Ctrl / Shift selection; a debounced single-click preview vs double-click open (demonstrating how to distinguish the two when the native ordering is `click → click → dblclick`); a right-click **context menu** (Open / Copy path / Rename / Delete / New folder / Select all, closes on outside-click / Escape); and working New folder / Rename / Delete that mutate the data and restore tree expansion via `getExpandedPaths()` / `setExpandedPaths()`. Rendered as the first card on the existing Custom Layout page (the iOS-Files and Notepad++ demos remain below). Smoke-covered by `e2e/windows-explorer.spec.ts` (12 cases).
