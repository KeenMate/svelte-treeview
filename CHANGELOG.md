...
- **`copyNodeTransformationCallback(data, ctx) => T`** — per-node clean/redact at snapshot time, so transient/sensitive fields never travel on the shared (cross-tree) clipboard. Demonstrated live in `/examples/tree-editor` (a `secret` token rendered as a badge, redacted on copy).
