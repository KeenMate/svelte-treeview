...
### Changed
- **The checkbox checkmark and indeterminate dash are now mask glyphs, not CSS-border shapes.** `.stv__checkbox-box::after` previously drew the check with rotated borders and the dash with a `border-bottom`; both now render `--stv-icon-check` / `--stv-icon-indeterminate` as a `currentColor` mask tinted by `--stv-checkbox-checkmark-color` — themeable via the base contract, matching the disclosure glyph. `--stv-checkbox-checkmark-width` no longer affects the glyph (stroke width is baked into the SVG) — kept as a no-op for back-compat.
- **The four parallel toggle-glyph CSS-class families were collapsed into the single `iconSet` knob (see Added).** `stv__toggle-icon--expand-alt` / `--collapse-alt` / `--expand-plus` / `--collapse-minus` / `--expand-arrow` / `--collapse-arrow` are **removed**; there is now one `stv__toggle-icon--expand` disclosure class whose picture is decided by the `--stv-icon-*` variables. `Node.svelte` renders that one class + an `.expanded` state marker; for the built-in glyph, swap/rotate is CSS-driven (via `data-toggle-icon-mode` / `data-icon-set`) rather than class-name swapping. `expandIconClass` / `collapseIconClass` are **deprecated** but remain a working escape hatch for a *custom* glyph class (e.g. FontAwesome), which isn't a CSS mask: in `swap` mode `Node.svelte` still swaps `expandIconClass` ↔ `collapseIconClass`, and in `rotate` mode the custom glyph still rotates (rotation is keyed off the shared `.stv__toggle-icon.stv__clickable`, not `--expand`). Consumers who set `expandIconClass="stv__toggle-icon--expand-plus"` etc. to pick a *built-in* glyph should switch to `iconSet`.
- **All remaining built-in glyphs are now Lucide icons drawn via CSS `mask`, so they inherit `currentColor` and are swappable per-instance.** Three holdouts were converted off unicode/emoji: the **context-menu submenu arrow** (`▸` → Lucide `chevron-right`, via `--stv-icon-chevron-right`), the **drag-denied badge** (the `🚫` emoji — which rendered differently per OS and couldn't be recoloured — → Lucide `ban`, via `--stv-icon-ban`, tinted by `--stv-drag-denied-badge-color`), and the **scroll-highlight direction markers** (`⇨`/`⇦` → Lucide `arrow-right`/`arrow-left`, via `--stv-icon-arrow-right`/`--stv-icon-arrow-left`). This matches the toggle-icon set (chevron/square-plus/minus/arrow) and the new node-info ⓘ, which were already Lucide masks. Deliberately left as-is: the CSS-drawn checkbox check/dash (conventional, scales with `--stv-checkbox-size`), the highlight-bracket `❯`/`❮` decorative style, and the opt-in `--leaf` dot. (The four parallel toggle-glyph class families this bullet originally also mentioned — `--*-alt` / `--*-plus` / `--*-arrow` — were collapsed into the single `iconSet` knob later in this cycle; see below.)
...
- **Examples showcase consolidated, normalized, and given coded section headings.** Ported the presentation overhaul from `@keenmate/web-multiselect`. (1) **Merged four overlapping pages** into their siblings: `expand-collapse` → **Basic Usage** (`/examples/basic`, now the quick "what is this library" tour, `BU01–07`), `silent-highlight` → **Interaction** (`IN05 · Silent State Restore`), `branch-operations` → **Data Structures** (`DS05/06 · Branch Operations` + live org-chart demo), and `scenarios` → **Tree Editor** (`TE05 · Business Scenarios` — the five DB-workflow tabs). Those four routes and the now-orphaned `ExampleHeader.svelte` are deleted; the landing registry drops from 15 to 11 cards. (2) **Coded section headings** — every page prefixes its `<h2>`s with a per-page code (`BU01 ·`, `IN02 ·`, `DD03 ·`, …) and dropped the emoji from every `<h1>`, so the outline reads consistently. (3) **Shared CSS extraction** — the reusable example vocabulary that pages were each redefining (`.reference-table`, `.log-panel`/`.log-entry`, `.subsection`, `.badge.new`, `.muted`, `kbd`, `.metric(s)`, `.tabs`/`.tab`, `.control-row`, `.unsaved-badge`, `.demo-card`, `.card-grid`, a `pre code` reset) is now defined once in `static/examples-shared.css`; per-page `<style>` blocks were normalized onto it, keeping only genuinely-unique visuals local (custom-layout's iOS/dendrogram UI, theming's brand-variant `--base-*` wrappers, basic's indentation-demo readout). Showcase-only — no change to the published library.

### Added
- **The checkbox checkmark + tri-state dash now chain to the shared `--base-icon-*` contract.** New `--stv-icon-check` / `--stv-icon-indeterminate` (each `var(--base-icon-check|-indeterminate, <inline Lucide fallback>)`) drive the `.stv__checkbox-box::after` glyph, so a theme reskins the checkmark and the partial-selection dash across every KeenMate component from one place. Manifest gains the two base tokens + two `--stv-icon-*` vars.
- **`iconSet?: 'chevron' | 'triangle' | 'plus-minus' | 'arrow'` (default `'chevron'`) — one knob picks the disclosure glyph, and it chains to the shared `--base-icon-*` contract.** Replaces the old *four parallel CSS-class families* (`--expand`/`--collapse`, `--expand-alt`/`--collapse-alt`, `--expand-plus`/`--collapse-minus`, `--expand-arrow`/`--collapse-arrow`) with **one** `--stv-icon-expand` / `--stv-icon-collapse` variable set that `iconSet` simply *re-points* — via a `data-icon-set` attribute on `.stv__container` — while the rendering rule and the single `stv__toggle-icon--expand` class never change. This is the pure-admin "one canonical structure, rigidity over flexibility" rule (a wrapper/generator emits one DOM tree per component) applied here, and it matches the sibling `@keenmate/web-treeview`. Each set chains its glyph to a `--base-icon-*` token (`chevron` → `--base-icon-chevron`, `triangle` → `--base-icon-caret-down`, `plus-minus` → `--base-icon-expand`/`--base-icon-collapse`), so setting one base token reskins the disclosure glyph across every KeenMate component at once; `arrow` uses an inline Lucide glyph (no base equivalent). Threaded through `<Tree>` + `update()`; demo `/examples/theming` TH04 (live picker + a `--base-icon-chevron` reskin toggle) & TH05 (reference table); guarded by `e2e/icon-set.spec.ts` + fixture `/test/icon-set`.
- **Per-set glyph rotation tokens — `--stv-icon-rotate-collapsed` / `--stv-icon-rotate-expanded`.** A base glyph's native orientation varies, so each `icon-set` declares its own collapsed→expanded offsets (chevron/arrow point right → `0° → 90°`; the filled caret points down → `-90° → 0°`). Mirrors the rotation tokens `@keenmate/web-multiselect` and `@keenmate/web-daterangepicker` needed for the same reason. `plus-minus` is inherently a swap set (a rotated +/− is meaningless), so it glyph-swaps regardless of `toggleIconMode` and its rotation is a no-op.
- **`nodeTitleOverflow?: 'wrap' | 'ellipsis' | 'info'` (default `'wrap'`) — control what the built-in node label does when it's wider than the row.** `wrap` (current behaviour) lets the label wrap onto multiple lines; `ellipsis` keeps it on one line and clips with `…`; `info` is `ellipsis` PLUS a trailing circled-ⓘ affordance that appears **only on rows whose label is actually clipped** — clicking it reveals the full label in a floating popover (the touch/no-hover substitute for the native `title` tooltip, which `info` mode also sets on clipped rows). Ported from the fullscreen clipped-label affordance in `@keenmate/web-multiselect`, adapted for an always-inline tree. Wrap/ellipsis are pure CSS, gated by `data-node-title-overflow` on `.stv__container` (flex `min-width: 0` fixes so the ellipsis engages); `info` adds a per-node clip measure (`scrollWidth > clientWidth`) that re-runs on content change and — via the shared `ResizeObserver` from the new responsive vendor — on container/indent/font resize, so the ⓘ appears and disappears as the box narrows and widens. The reveal is placed with `@floating-ui/dom` (already a dependency), `position: fixed` so it escapes the tree's overflow clip, and dismisses on a second click / outside pointer / scroll / Escape; clicking the ⓘ never selects the node. Only the default label path participates — a `nodeTemplate` snippet owns its own layout. The ⓘ glyph is the **Lucide `info` icon** drawn via a CSS `mask` (inherits the button's `color`), swappable wholesale through `--stv-icon-info`. New CSS hooks: `.stv__node-info`, `.stv__node-info-reveal` (+ `--stv-icon-info`, `--stv-node-info-*` / `--stv-node-info-reveal-*` variables). Threaded through `<Tree>` + `update()`; demo `/examples/responsive` RS03 (mode switch over the ISCO titles); guarded by `e2e/node-title-overflow.spec.ts` + fixture `/test/node-title-overflow`.
- **Responsive container/viewport/device signal — the tree now surfaces a live measure of its own space (and the device) so consumers can adapt settings to it.** The device/viewport/container-size engine from `@keenmate/web-components-core` (`environment.ts` + `element-size.ts`) is **transferred 1:1** into `src/lib/vendor/environment` (zero-dependency, SSR-safe, ref-counted over one shared `matchMedia`/`ResizeObserver`; the overlay/fullscreen tier is deliberately NOT taken — the tree renders inline). A runes bridge (`src/lib/core/responsive.svelte.ts`) turns each imperative `subscribe → unsubscribe` into reactive `$state` (the unsubscribe becomes the `$effect` cleanup): `containerSize(getEl)` and `environmentState()`. `<Tree>` exposes the signal **signal-only** (it changes none of its own rendering) two ways: a **bindable `containerSize`** (`ElementSize` — declarative `$derived` reads) and an **`onContainerResize(size, deviceClass)`** callback (imperative; `deviceClass` via `classifyDevice` — capability + physical size, orientation-robust). The emitted box is a fresh plain snapshot with a last-emitted guard, and the bridge uses `$state.raw`, so a consumer's `bind:` can't alias the tree's reactive state into a write-back loop. The raw primitives (`observeEnvironment`/`observeViewport`/`observeElementSize`/`classifyDevice`/`configureBreakpoints`/`getEnvironment`/`TABLET_MIN_SHORT_SIDE` + types) are re-exported for app-level (non-Svelte) AB10-style adaptation. Demo `/examples/responsive` (`RS01–03`); guarded by `e2e/responsive.spec.ts` + fixture `/test/responsive`; vendored modules keep their upstream unit tests (jsdom-scoped, 38 tests). Groundwork for a future `@keenmate/svelte-components-core`.
...
- **Floating "on this page" chapter navigation across the examples showcase** (`src/routes/ChapterNav.svelte`, ported from web-multiselect). A tab pinned to the right edge expands into a jump list: on an example page it auto-scans the `<h2>` sections (assigning scroll-margin ids) with scroll-spy and re-scans on client-side navigation; on the landing page it renders a cross-page list fed from the same `examples` registry. Positioned with `@floating-ui/dom` (already a dependency). Mounted once in `examples/+layout.svelte` (in-page mode) and via `<ChapterNav items={…} />` on `/` (cross-page mode).
- **The examples showcase now builds and serves as a Docker image** — mirroring the `@keenmate/web-multiselect` setup, adapted for SvelteKit. A multi-stage `Dockerfile` builds the static site (`npm ci` → `generate-constants` + `build:showcase` → `build/`, skipping the library `prepack`) and serves it from `nginx:alpine`; a separate `nginx.conf` silently drops vulnerability-scanner traffic (`444` by UA / probe-path / probe-extension), long-caches the hashed `_app/immutable/` assets, and uses `try_files $uri $uri.html $uri/ /index.html` (prerendered pages + SPA fallback). `make image-build` / `image-run` / `image-stop` / `image-clean` drive it, with a configurable container runner (`.makefile.env`, `DOCKER_RUNNER` default `podman`) and served port (`IMAGE_PORT` default `17780`). No version-badge or dev-entry patching needed — SvelteKit bakes the version in via `constants.generated.ts` and bundles all sources.
...
- **Blocked-action feedback for drag & drop — `onNodeDragDenied` / `onNodeDropDenied` events + a built-in held indicator (`shouldIndicateUndraggable`).** Trying to touch-drag a locked node used to do NOTHING — `_onTouchStart` saw `getIsDraggable === false` and returned silently, so the finger just felt broken. Now a deliberate long-press on a non-draggable node (a tap/scroll still cancels it via the existing move-threshold) plays a "can't move this" reaction: a 🚫 no-entry badge that **stays on the row for as long as the finger is held** (torn down on release) with a gently pulsing red tint, plus a distinct haptic double-buzz (`navigator.vibrate([30,25,30])` — a real drag start is a single buzz). Its target-side twin fires when a drop is **rejected because the target refuses it** (`isDropAllowed` false / `getIsDropAllowedCallback`), flashing the same 🚫 briefly on the refusing node (not fired when a `shouldEnableTreeDropZone` catches the forwarded drop instead). Two new fire-and-forget events carry the node's `NodeRef` for custom toasts/snackbars: **`onNodeDragDenied`** (source-side) and **`onNodeDropDenied`** (target-side); both fire regardless of the built-in visual. **`shouldIndicateUndraggable?: boolean` (default `true`)** toggles just the built-in haptic + icon. All threaded through `<Tree>` and `update()`; CSS hook `--stv-drag-denied-flash-bg` recolors the pulse. The drag-drop example trees gained `displayValueMember="name"` so the touch ghost shows a real label instead of the `[N/A]` fallback.
- **`touchDragDelay?: number` (default `300`) — the long-press hold before a touch-drag engages is now configurable.** Was a hardcoded `setTimeout(…, 300)` in the touch handler; exposed as a prop (threaded through `<Tree>`, a live `$effect` sync, and `update()`) so consumers can dial in a snappier (`150`) or more deliberate, iOS/Android-like (`500`) feel.
- **`cascadeSelectPolicy?: CascadeSelectPolicy` (default `'rolled-up'`) — a second, orthogonal checkbox knob controlling which paths the selection EMITS in cascade mode.** `checkboxMode` already controls the cascade BEHAVIOUR (does checking a branch fill its subtree); this new prop controls the projected OUTPUT — what `bind:selectedPaths` and `onSelectionChange` report — without changing what's checked on screen. Only applies when `checkboxMode === 'cascade'` (independent mode emits the raw checked set). Three policies: **`'rolled-up'`** (minimal cover — a fully-checked subtree collapses to its root, a partially-checked branch emits its individually-checked descendants), **`'leaves'`** (only the checked leaf nodes), **`'all'`** (every fully-checked node, branches + leaves). Internally the controller keeps the CANONICAL checked set (every fully-checked node) on `#selectedPaths` and derives an emitted projection `#emittedPaths` via a new pure `_projectSelection`; the bindable prop now exposes `controller.emittedPaths`, while `controller.selectedPaths` (getter) still returns the canonical set for internal/back-compat reads. A parent SET of the prop is treated as a projection and expanded back into the canonical checked state (cascade-down via `setSelectedPaths`), so binding round-trips are stable under every policy. Live policy or mode switches re-project the unchanged checked state. Threaded through `<Tree>` and `update()`; new exported `CascadeSelectPolicy` type. Ports the `cascade-select-policy` knob from `@keenmate/web-multiselect`. Guarded by `e2e/checkbox-policy.spec.ts` (all three projections, live policy switching, round-trip, unchecking) via the `/test/checkbox-policy` fixture; a live picker (both knobs on one tree) added to `/examples/interaction`.

### Changed
- **Default cascade emission is now `'rolled-up'`, not the full checked set.** Before this release, cascade-mode `selectedPaths` always exposed *every* fully-checked node (leaves + branches) — equivalent to the new `'all'` policy. It now defaults to `'rolled-up'`, so a consumer binding `selectedPaths` in cascade mode receives the minimal cover (subtree roots) instead of the flattened set. Pass `cascadeSelectPolicy="all"` to restore the previous output. (RC cycle — treated as implicitly breaking.)

### Fixed
- **Touch drag-and-drop was dead in touch environments (most visibly Chrome DevTools device emulation): the native `draggable` attribute swallowed the synthetic touch stream.** With touch emulation on, `touchstart` fired and the long-press engaged (ghost appeared), but the very first move delivered NO `touchmove`/`touchend` — Chrome tried to hand the gesture to its mouse-driven native HTML5 drag engine (there's no real mouse under emulation, so native DnD never started *and* the touch events were suppressed), freezing the drag mid-flight. Fixed in `Node.svelte`: on `touchstart` the node's `draggable` is set `false` **synchronously** (before the move that triggers the hijack) and restored on `touchend`/`touchcancel`, so the touch-drag handlers receive the full stream. Scoped to actual touch gestures — a mouse never fires `touchstart`, so mouse-drag on a touchscreen laptop is unaffected. (On real phones this collision can't occur; mobile browsers ignore `draggable` for touch input.)
- **Virtual scroll: the last rows were unreachable and near-bottom scrolling jittered/snapped back.** Virtual scroll assumes a UNIFORM row height and sizes its scroll spacer as `count × rowHeight`, but flat rows added a `margin-top: 2px` `flatGap` on every level-transition row (a first child sitting one level deeper than the previous row). That margin is invisible to `getBoundingClientRect()` — the height the row-measurement reads — so each gap row was really 2px taller than the spacer accounted for. Across a tree with many level transitions (e.g. a filtered list of parents each opening their children) the un-counted 2px margins accumulated into tens-to-hundreds of pixels, so the spacer under-estimated real content: the last rows fell past the scroll range (couldn't reach the bottom) and every scroll tick near the end fought the layout (the "jumping"). The variable `flatGap` margin is now forced OFF in the virtual-scroll branch (`Tree.svelte`), keeping rows truly uniform so the spacer is exact; the gap still applies in plain-flat and recursive modes, which aren't virtualized. NOTE: fixed-height virtualization still requires uniform row content — rows whose height varies (e.g. wrapping labels) remain a limitation; set `virtualRowHeight` or keep row content single-line. Guarded by `e2e/virtual-scroll-bottom.spec.ts`.
