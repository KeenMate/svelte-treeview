<script lang="ts">
	...
	const searchableData = [
		...
		},
		{
			...
			name: 'Reports',
			type: 'folder',
			description: 'Monthly and quarterly business reports'
		},
		{
			id: '1.2.1',
			path: '1.2.1',
		...
		},
		{
			id: '1.2.2',
			path: '1.2.2',
			name: 'User Analytics Dashboard.xlsx',
			type: 'file',
			description: 'User behavior analytics and metrics dashboard'
		},
		{
			id: '2',
			path: '2',
		...
		},
		{
			...
			type: 'folder',
			description: 'Images for marketing campaigns and social media'
		},
		{
			id: '2.1.1',
			path: '2.1.1',
		...
		},
		{
			...
			type: 'folder',
			description: 'Company logo in different formats and colors'
		},
		{
			id: '2.2',
			path: '2.2',
			name: 'Video Content',
			type: 'folder',
			description: 'Promotional and educational video content'
		},
		{
			id: '2.2.1',
			path: '2.2.1',
			name: 'Tutorial Videos.mp4',
			type: 'file',
			description: 'Step-by-step user tutorials and guides'
		},
		{
			id: '3',
			path: '3',
			name: 'Development',
			type: 'folder',
			description: 'Source code, documentation, and development resources'
		},
		{
			id: '3.1',
			path: '3.1',
			name: 'Frontend Code',
			type: 'folder',
			description: 'React, Vue, and Svelte component libraries'
		},
		{
			id: '3.2',
			path: '3.2',
			name: 'Backend API',
			type: 'folder',
			description: 'REST API endpoints and microservices architecture'
		},
		{
			id: '3.3',
			path: '3.3',
	...
	const sortCallback = (items: any[]) => {
		return items.sort((a, b) => {
			if (a.data.type !== b.data.type) {
				...
				return (
					(typeOrder[a.data.type as keyof typeof typeOrder] || 3) -
	...
	function performSearch() {
		if (treeRef && searchQuery.trim()) {
			searchResults = treeRef.searchNodes(searchQuery.trim());
		} else {
			searchResults = [];
		}
	}
...
<div class="container-fluid">
	...
	>
		...
		{#snippet controls()}
			<div class="form-check mb-3">
				<input
					...
					id="enableSearch"
				/>
				<label class="form-check-label" for="enableSearch"> Enable Internal Search Index </label>
			</div>
			...
			<div class="form-check mb-3">
				<input
					...
					id="showDebug"
				/>
				<label class="form-check-label" for="showDebug"> Show Debug Information </label>
			</div>
		...
		{#snippet description()}
			...
			<p>
				Uses <code>requestIdleCallback</code> to build search index without blocking UI rendering.
			</p>

			<h6>Performance Benefits</h6>
			<p>
				Tree renders immediately while search indexing happens during browser idle time, preventing
	...
	>
		{#snippet demo()}
			...
			<div class="bg-light p-3 rounded">
				...
				{#if useCustomCallback}
					<code
						>getSearchValueCallback = (node) => `${node.data.name} ${node.data.description} ${node
							.data.type}`</code
					>
				{:else}
		...
		{#snippet description()}
			...
			<p>
				<code>getSearchValueCallback</code> - Custom function to combine multiple fields or transform
			...
			<p>
				Internally uses FlexSearch for fast, fuzzy matching with stemming and phonetic search
				capabilities.
			</p>
		{/snippet}
	</ShowcaseSection>
	...
	<ShowcaseSection
		title="searchNodes() Method"
		subtitle="Programmatically search and retrieve matching nodes"
	>
		{#snippet demo()}
			<div class="mb-3">
				<div class="input-group">
					...
					/>
					<button class="btn btn-primary" onclick={performSearch}> Search Nodes </button>
				</div>
			</div>
			...
			{#if searchResults.length > 0}
				<div class="alert alert-success">
					<strong
						>Found {searchResults.length} matching node{searchResults.length > 1
							? 's'
							: ''}:</strong
					>
				</div>
						...
						<div class="list-group-item">
							<div class="d-flex align-items-start">
								...
								<div class="flex-grow-1">
									...
									<small
										><strong>Path:</strong>
										{node.path} | <strong>Type:</strong>
										{node.data.type}</small
									>
								</div>
							</div>
						</div>
			...
			{:else if searchQuery.trim()}
				<div class="alert alert-warning">
					No nodes found for query: "{searchQuery}"
				</div>
			{/if}
		...
		{#snippet controls()}
			<div class="alert alert-info">
				<small>
					<strong>Try searching for:</strong><br />
					• "website" (project name)<br />
					• "financial" (in description)<br />
					• "database" (project name)<br />
					• "video" (content type)<br />
					• "api" (technical term)
				</small>
			</div>
		{/snippet}
		...
		{#snippet description()}
			...
			<p>
				The <code>searchNodes(searchText)</code> method returns an array of matching nodes without filtering
				the tree display.
			</p>
...
// Search nodes programmatically
const results = treeRef.searchNodes("query");
console.log(`Found $&#123;results?.length&#125; nodes`);</code
				></pre>

			<h6>Difference from filterNodes()</h6>
			<ul class="small">
				<li>
					<strong>searchNodes()</strong>: Returns matching nodes array, tree display unchanged
				</li>
				<li><strong>filterNodes()</strong>: Filters tree display to show only matches</li>
			</ul>

			<h6>Use Cases</h6>
			<ul class="small">
				<li>Build custom search interfaces</li>
