...
test.describe('two-tree drag (source → target)', () => {
	test('target starts empty (no nodes); empty-state drop area is rendered', async ({ page }) => {
		...
		// what's always present when the tree is empty is the .ltree-empty-state
		// drop zone that catches the dragenter.
		await expect(target.locator('.ltree-node[data-tree-path]')).toHaveCount(0);
		await expect(target.locator('.ltree-empty-state')).toHaveCount(1);
	});
	...
	test('dragging from source onto the empty target inserts and records drop event', async ({
		...
		const emptyDropArea = targetBox.locator('.ltree-empty-state');
		...
		await expect(targetBox.locator('.ltree-node[data-tree-path]')).toHaveCount(1);
	...
	test('Reset button clears target and zeroes drop counter', async ({ page }) => {
		...
		await expect(targetBox.locator('.ltree-node[data-tree-path]')).toHaveCount(0);
	});
});
...
test.describe('touch drag', () => {
	...
	test('long-press (>300ms) on a node creates the touch-ghost element', async ({ page }) => {
		...
		await expect(page.locator('.ltree-touch-ghost')).toHaveCount(1);

		await dispatchTouch(page, src, 'touchend', x, y);
		await expect(page.locator('.ltree-touch-ghost')).toHaveCount(0);
	});
